...
describe("GET /todos/:id", () => {
	...
	it("should not return todo doc created by other user", (done) => {
		request(app)
		 .get(`/todos/${todos[1]._id.toHexString()}`)//toHexString() konvertuje ObjectID u Sting
		 .set("x-auth", users[0].tokens[0].token)
		 .expect(400)
		 .end(done);
	});
...
describe("PATCH /todos/:id", () => {
	...
	it("should not update todo as other user", (done) => {
		var hexId = todos[0]._id.toHexString();
        var text = "Updated Text in first item";
...
describe("POST /users", () => {
	...
	it("should not create a user if email is in use", (done) => {//uzima se mail iz seed fajla
...
describe("DELETE /users/me/token", () => {
	it("should delete auth token when logging out", (done) => {
		 request(app)
